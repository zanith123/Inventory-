        ...
        if ($name === '' || $sku === '') {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $pdo->prepare('INSERT INTO products
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,?,0)');
                $stmt->execute([
                    $name, $sku, trim($_POST['barcode']) ?: $sku,
...
$search = trim($_GET['q'] ?? '');
$sql = 'SELECT p.*, c.name AS category_name, s.name AS supplier_name, u.name AS unit_name
        FROM products p
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h4 class="mb-1"><?= __('product_title') ?></h4>
    <p class="text-secondary small mb-0">Manage items, pricing, SKUs, and barcode labels</p>
  </div>
    ...
    <table class="table mb-0 align-middle">
      <thead class="table-light">
        <tr>
          ...
          <th><?= __('common_supplier') ?></th>
          <th><?= __('product_col_cost') ?></th>
          <th><?= __('product_col_price') ?></th>
          <th><?= __('product_col_margin') ?></th>
          <th><?= __('product_col_stock') ?></th>
      ...
      <tbody>
        ...
        <?php foreach ($products as $i => $p):
          ...
          $barcodeCode = !empty($p['barcode']) ? $p['barcode'] : $p['sku'];
        ?>
        <tr>
          <td><?= $i + 1 ?></td>
          <td>
            ...
            <div class="d-flex align-items-center gap-1 mt-1">
              ...
              <button class="btn btn-link btn-sm p-0 text-secondary ms-1" data-bs-toggle="modal" data-bs-target="#barcodeModal<?= $p['id'] ?>" title="View & Print Barcode">
          ...
          <td class="mono fw-semibold" style="color:<?= $margin >= 30 ? 'var(--good)' : ($margin >= 15 ? 'var(--warn)' : 'var(--danger)') ?>;"><?= $margin ?>%</td>
          <td><span class="badge-stock <?= $low ? 'badge-low' : 'badge-normal' ?>"><?= number_format($p['current_stock']) ?> <?= htmlspecialchars($p['unit_name'] ?? __('common_pcs')) ?></span></td>
          <td class="text-end">
            <button class="btn btn-sm btn-outline-primary"
                    data-bs-toggle="modal" data-bs-target="#editModal<?= $p['id'] ?>" title="Edit Product">
        ...
        <!-- Barcode Print Modal -->
        <div class="modal fade" id="barcodeModal<?= $p['id'] ?>" tabindex="-1">
          <div class="modal-dialog modal-sm">
            <div class="modal-content text-center">
              <div class="modal-header">
                <h5 class="modal-title font-monospace fs-6"><i class="bi bi-upc me-1"></i> Barcode Label</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
              </div>
              <div class="modal-body">
                <div class="printable-area p-3 bg-white text-dark rounded">
                  <h6 class="fw-bold mb-1" style="font-size:14px;"><?= htmlspecialchars($p['name']) ?></h6>
                  <div class="my-2" style="max-width:220px; margin: 0 auto;">
                    <?= generate_barcode_svg($barcodeCode, 60) ?>
                  </div>
                  <div class="d-flex justify-content-between font-monospace small border-top pt-2 mt-2">
                    <span>Price: <strong>$<?= number_format($p['sale_price'], 2) ?></strong></span>
                    <span>SKU: <strong><?= htmlspecialchars($p['sku']) ?></strong></span>
                  </div>
                </div>
              </div>
              <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-primary btn-sm w-100" onclick="window.print()">
                  <i class="bi bi-printer me-1"></i> <?= __('v2_print') ?>
                </button>
              </div>
            </div>
          </div>
    ...
    <div class="modal-content">
      <form method="post">
        ...
        <div class="modal-body">
          ...
          <div class="row">
            <div class="col-6 mb-3"><label class="form-label"><?= __('product_sku') ?></label>
              <input type="text" name="sku" class="form-control" required placeholder="e.g. LAP-DELL-01"></div>
            <div class="col-6 mb-3"><label class="form-label"><?= __('product_barcode') ?></label>
              <input type="text" name="barcode" class="form-control" placeholder="Optional barcode"></div>
          </div>
          <div class="row">
            <div class="col-4 mb-3"><label class="form-label"><?= __('common_category') ?></label>
              <select name="category_id" class="form-select">
          ...
          <div class="row">
            ...
            <div class="col-4 mb-3"><label class="form-label"><?= __('product_min_stock') ?></label>
              <input type="number" name="min_stock" class="form-control" value="5"></div>
          </div>
          <div class="mb-3"><label class="form-label"><?= __('common_note') ?></label>
